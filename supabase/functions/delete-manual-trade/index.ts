...
serve(async (req) => {
  ...
  const functionName = 'delete-manual-trade';
  ...
  try {
    ...
    const { data: trade, error: fetchTradeError } = await supabaseAdmin
      .from('manual_trades')
      .select('pair, asset_amount, status, strategy_type')
      .eq('id', tradeId)
    ...
    let shouldAttemptBinanceSell = true;
    ...
    if (trade.status === 'active' && trade.asset_amount && trade.asset_amount > 0) {
      ...
      try {
        ...
        if (quantityToSell === 0) {
          ...
          adjustedQuantity = adjustQuantity(quantityToSell, stepSize);
    ...
    } else if (trade.status === 'awaiting_dip_signal') {
      console.log(`[${functionName}] Trade ${tradeId} está esperando una señal de dip. No hay activos para vender.`);
      shouldAttemptBinanceSell = false;
    }
